(function () {
	...
	function ngGallery( $document, $timeout, $q, $templateCache ) {
		...
		//	'  <a class="nav-left" ng-click="prevImage()"><i class="fa fa-angle-left"></i></a>' +
			...
			'  <img ondragstart="return false;" src="{{images[0].img}}" draggable="false" ng-src="{{ img }}" ng-click="nextImage()"  ng-show="true" class="effect"  />' +
		...
		//	'  <span class="info-text">{{ index + 1 }}/{{ images.length }} - {{ description }}</span>' +
			...
			'      <div ng-repeat="i in images" >' +
			'        <img ng-src="{{ i.thumb }}" ng-class="{\'active\': index === $index}" ng-click="changeImage($index)" border="3"/>' +
			'      </div>' +
			'    </div>' +
			'  </div>' +
			'</div>'
